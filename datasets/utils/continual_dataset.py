...
class ContinualDataset(object):
    ...
    def get_all_classes(self):
        ...
        start_c = 0 #self.N_CLASSES_PER_TASK * task_idx if isinstance(self.N_CLASSES_PER_TASK, int) else sum(self.N_CLASSES_PER_TASK[:task_idx])
        end_c = self.N_CLASSES_PER_TASK * self.N_TASKS if isinstance(self.N_CLASSES_PER_TASK, int) else sum(self.N_CLASSES_PER_TASK)
...
def getAllLoaders(train_dataset: Dataset, test_dataset: Dataset,
    ...
    # Split the dataset into tasks
    start_c, end_c = setting.get_all_classes()
