...
class BiLoRA_Local(Attention_LoRA):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0, proj_drop=0, r=64, n_tasks=10, n_frq=3000, local_indices=[]):
        super().__init__(dim, num_heads, qkv_bias, qk_scale, attn_drop, proj_drop, r, n_tasks)
        self.device = torch.device("cuda")
        self.bilora_indices = torch.tensor(local_indices).to(self.qkv.weight.device)
        self.bilora_mask = torch.stack([self.bilora_indices // dim, self.bilora_indices % dim], dim=0)
        self.dim = dim
        self.n_frq = n_frq
        self.coef_k = nn.Parameter(torch.randn(self.n_frq), requires_grad=True).to(self.qkv.weight.device)
        self.coef_v = nn.Parameter(torch.randn(self.n_frq), requires_grad=True).to(self.qkv.weight.device)
    ...
    def get_delta_w_k(self, alpha=300):
        F = torch.zeros(self.dim, self.dim).to(self.qkv.weight.device)
        F[self.bilora_mask[0,:], self.bilora_mask[1,:]] =  self.coef_k
        return torch.fft.ifft2(F, dim=(-2,-1)).real * alpha
    
    def get_delta_w_v(self, alpha=300):
        F = torch.zeros(self.dim, self.dim).to(self.qkv.weight.device)
        F[self.bilora_mask[0,:], self.bilora_mask[1,:]] =  self.coef_v
        return torch.fft.ifft2(F, dim=(-2,-1)).real * alpha
...
class BiLoRA_Global(Attention_LoRA):
    ...
    def start_task(self, global_indices):
        self.global_indices.append(global_indices)
    
    def get_delta_w_k(self, task, alpha=300):
        global_indices = torch.tensor(self.global_indices[task]).to(self.qkv.weight.device)
        indices = torch.stack([global_indices // self.dim, global_indices % self.dim], dim=0)
        F = torch.zeros(self.dim, self.dim).to(self.qkv.weight.device)
        F[indices[0,:], indices[1,:]] =  self.coef_k[task]
        return torch.fft.ifft2(F, dim=(-2,-1)).real * alpha
    ...
    def forward(self, x, **kwargs):
        ...
        weight_k = torch.stack([self.get_delta_w_k(t) for t in range(len(self.global_indices))], dim=0).sum(dim=0)
        weight_v = torch.stack([self.get_delta_w_v(t) for t in range(len(self.global_indices))], dim=0).sum(dim=0)
        k = k + F.linear(x, weight_k).reshape(B, N, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
        v = v + F.linear(x, weight_v).reshape(B, N, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
...
class BiLORA_MoE(Attention_LoRA):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., r=64, n_tasks=10, n_frq=3000, num_experts=32, topk=5, use_expert_weights=False):
        super().__init__(dim, num_heads, qkv_bias, qk_scale, attn_drop, proj_drop, r, n_tasks)
        self.num_experts= num_experts
        self.n_tasks = n_tasks
